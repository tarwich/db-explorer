...
export const ConnectionTab = forwardRef<
  HTMLFormElement,
  {
    ...
    onProcessingTablesChange?: (processingTables: Set<string>) => void;
  }
>(
  (
    { connectionId, onDelete, onConnectionIdChange, onProcessingTablesChange },
    ref
  ) => {
    ...
    const autoAssignAllMutation = useMutation({
      mutationFn: () =>
        autoAssignAllTables({
          ...
          onProgress: (completed, total, tableName, processingTables) => {
            // Update processing state with all tables currently being processed
            onProcessingTablesChange?.(new Set(processingTables));
          },
        }),
      onError: (error) => {
        ...
        onProcessingTablesChange?.(new Set());
      },
      onSuccess: (result) => {
        toast({
